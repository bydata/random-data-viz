...
Each vendor's market share is coded in a column. In total, the market shares from `r n_vendors` vendors are available in the dataframe.
...
{ggpubr} is a great package for creating several chart types without all the details of {ggplot2}, including [donut charts](https://rpkgs.datanovia.com/ggpubr/reference/ggdonutchart.html). Since we will be making some customizations for our animation, we will create the plot from scratch in {ggplot2}, though.
